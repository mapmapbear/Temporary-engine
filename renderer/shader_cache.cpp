...
RHIShader* ShaderCache::GetShader(const std::string& file, const std::string& entry_point, const std::string& profile, const std::vector<std::string>& defines)
{
    RHIShaderDesc desc;
    desc.file = file;
    ...
    RHIShader* pShader = CreateShader(file, entry_point, profile, defines);
    if (pShader != nullptr)
    {
...
RHIShader* ShaderCache::CreateShader(const std::string& file, const std::string& entry_point, const std::string& profile, const std::vector<std::string>& defines)
{
    ...
    RHIShader* shader = m_pRenderer->GetDevice()->CreateShader(desc, shader_blob, name);
    return shader;
}
