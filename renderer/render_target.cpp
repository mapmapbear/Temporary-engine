...
RenderTarget::RenderTarget(bool auto_resize, float size, const std::string& name)
{
    m_name = name;
...
bool RenderTarget::Create(uint32_t width, uint32_t height, RHIFormat format, RHITextureUsageFlags flags)
{
...
void RenderTarget::OnWindowResize(uint32_t width, uint32_t height)
{
    ...
    RHITextureDesc desc = m_pTexture->GetDesc();
    desc.width = (uint32_t)ceilf(width * m_size);
    desc.height = (uint32_t)ceilf(height * m_size);
