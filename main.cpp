  ...
  case WM_SIZE:
    if (isFirst) {
      if (wParam != SIZE_MINIMIZED) {
        Engine::GetInstance()->CallWindowResize(LOWORD(lParam), HIWORD(lParam));
      }
    }
    isFirst = true;
    return 0;
...
static std::string GetWorkPath() {
  ...
  DWORD size =
      WideCharToMultiByte(CP_ACP, 0, exe_file, -1, NULL, 0, NULL, FALSE);

  std::string work_path;
  work_path.resize(size);

  WideCharToMultiByte(CP_ACP, 0, exe_file, -1, (LPSTR)work_path.c_str(), size,
                      NULL, FALSE);
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
  ...
  HWND hwnd = CreateWindow(windowClass.lpszClassName, L"Render Playground",
