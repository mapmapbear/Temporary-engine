...
{
public:
	D3D12CommandList(RHIDevice* pDevice, RHICommandQueue queue_type, const std::string& name);
	~D3D12CommandList();
	...
	virtual void ResourceBarrier(RHIResource* resource, uint32_t sub_resource, RHIResourceState old_state, RHIResourceState new_state) override;
	...
	virtual void SetConstantBuffer(RHIPipelineType type, uint32_t slot, void* data, size_t data_size) override;
