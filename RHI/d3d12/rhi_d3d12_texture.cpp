...
d3d12_resource_desc(const RHITextureDesc &desc) {
...
D3D12Texture::D3D12Texture(D3D12Device *pDevice, const RHITextureDesc &desc,
...
bool D3D12Texture::Create() {
  ...
  HRESULT hr = pAllocator->CreateResource(
      ...
      &m_pAllocation, IID_PPV_ARGS(&m_pTexture));
  if (FAILED(hr)) {
    return false;
  }
...
D3D12_CPU_DESCRIPTOR_HANDLE D3D12Texture::GetRTV(uint32_t mip_slice,
  ...
  if (IsNullDescriptor(m_RTV[index])) {
    ...
    ID3D12Device *pDevice = (ID3D12Device *)m_pDevice->GetHandle();
    pDevice->CreateRenderTargetView(m_pTexture, &rtvDesc,
                                    m_RTV[index].cpu_handle);
  }

  return m_RTV[index].cpu_handle;
}

D3D12_CPU_DESCRIPTOR_HANDLE D3D12Texture::GetDSV(uint32_t mip_slice,
                                                 uint32_t array_slice) {
  ...
  if (IsNullDescriptor(m_DSV[index])) {
    ...
    ID3D12Device *pDevice = (ID3D12Device *)m_pDevice->GetHandle();
    pDevice->CreateDepthStencilView(m_pTexture, &dsvDesc,
                                    m_DSV[index].cpu_handle);
  }
...
uint32_t D3D12Texture::GetRequiredStagingBufferSize() const {
  ...
  uint64_t size;
  pDevice->GetCopyableFootprints(&desc, 0, subresource_count, 0, nullptr,
...
uint32_t D3D12Texture::GetRowPitch(uint32_t mip_level) const {
  ...
  pDevice->GetCopyableFootprints(&desc, mip_level, 1, 0, &footprint, nullptr,
                                 nullptr, nullptr);
